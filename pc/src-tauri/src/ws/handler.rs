            ...
            match msg {
                Message::Text(text) => {
                    ...
                    if authenticated_device_id.is_none() {
                                ...
                                if let (Ok(pubkey_bytes), Ok(qr_nonce_bytes), Ok(qr_sig_bytes)) = (
                                    ...
                                    if pubkey_bytes.len() == 32 && qr_nonce_bytes.len() == 32 {
                                        ...
                                        let is_active_nonce = {
                                            let inner = state.inner.lock().unwrap();
                                            inner
                                                .active_pairing_nonce
                                                .map(|n| n == qr_nonce_bytes.as_slice())
                                                .unwrap_or(false)
                                        };
                                        ...
                                        if !is_active_nonce {
                                            let err = Envelope::new_error(
                                                envelope.id,
                                                4,
                                                "Invalid or expired pairing nonce".to_string(),
                                            );
                                            let _ = tx
                                                .send(Message::Text(
                                        ...
                                        if !qr_sig_valid {
                                            let err = Envelope::new_error(
                                                envelope.id,
                                                5,
                                                "Invalid pairing signature".to_string(),
                                            );
                                            let _ = tx
                                                .send(Message::Text(
                                                    serde_json::to_string(&err).unwrap(),
                                        ...
                                        pairing_state = Some(PairingFlowState::ChallengeSent(
                                            IntermediatePairing {
                                        ...
                                        if let Some(h) = &app_handle {
                                            let _ = h.emit(
                                                "pairing-status",
                                                "Scanning complete. Verifying signature...",
                                            );
                                        }
                        ...
                        } else if envelope.t == "resp"
                            && matches!(pairing_state, Some(PairingFlowState::ChallengeSent(_)))
                        {
                                ...
                                match BASE64_URL_SAFE_NO_PAD.decode(s) {
                                    Ok(sig_bytes) => {
                                        if sig_bytes.len() == 64 {
                                            ...
                                            let verifying_key =
                                                VerifyingKey::from_bytes(&state_pair.pubkey_phone);
                                            ...
                                            match (signature, verifying_key) {
                                                (Ok(sig), Ok(key)) => {
                                                    match key.verify(&state_pair.nonce_pc, &sig) {
                                                        Ok(_) => true,
                                                        Err(e) => {
                                                            let nonce_hex: String = state_pair
                                                                .nonce_pc
                                                                .iter()
                                                                .map(|b| format!("{:02x}", b))
                                                                .collect();
                                                            let pubkey_hex: String = state_pair
                                                                .pubkey_phone
                                                                .iter()
                                                                .map(|b| format!("{:02x}", b))
                                                                .collect();
                                            ...
                                            tracing::error!(
                                                "Handshake: signature length is not 64: {}",
                                                sig_bytes.len()
                                            );
                                            false
                                        }
                                    ...
                                    Err(e) => {
                                        tracing::error!(
                                            "Handshake: failed to decode base64 signature: {:?}",
                                            e
                                        );
                                        false
                                    }
                            ...
                            if verified {
                                ...
                                let device_id =
                                    existing_id.unwrap_or_else(|| ulid::Ulid::new().to_string());
                                let token =
                                ...
                                if let Some(h) = &app_handle {
                                    let _ = h.emit(
                                        "pairing-status",
                                        "Fingerprint verified. Waiting for confirmation...",
                                    );
                                }

                                let verified_resp = Envelope::new_response(
                                    envelope.id,
                                    json!({
                                        "status": "verified",
                                        "device_id": device_id,
                                ...
                                let _ = tx
                                    .send(Message::Text(
                                        serde_json::to_string(&verified_resp).unwrap(),
                                    ))
                                    .await;
                            } else {
                                if let Some(h) = &app_handle {
                                    let _ = h.emit(
                                        "pairing-status",
                                        "Fingerprint mismatch. Pairing rejected.",
                                    );
                                }
                                let err = Envelope::new_error(
                                    envelope.id,
                            ...
                            && envelope.method.as_deref() == Some("pair_confirm")
                        {
                            if let Some(PairingFlowState::ChallengeVerified {
                                pair,
                                device_id,
                                token,
                            }) = pairing_state.take()
                            {
                                ...
                                if let Some(h) = &app_handle {
                                    let _ =
                                        h.emit("pairing-status", "Fingerprint verified. Paired!");
                                }

                                let paired_resp = Envelope::new_response(
                                    envelope.id,
                                    json!({
                                        "status": "paired",
                                        "device_id": device_id,
                                ...
                                let _ = tx
                                    .send(Message::Text(
                                        serde_json::to_string(&paired_resp).unwrap(),
                                    ))
                                    .await;
                            } else {
                                let err = Envelope::new_error(
                                    envelope.id,
                                    6,
                                    "Invalid pairing state for confirmation".to_string(),
                                );
                                let _ = tx
                                    .send(Message::Text(serde_json::to_string(&err).unwrap()))
                                    .await;
                            ...
                            match frame.stream_id {
                                ...
                                1 => {
                                    ...
                                    if !is_valid_request_id(request_id) {
                                        tracing::error!(
                                            "Invalid request_id detected in video stream: {:?}",
                                            request_id
                                        );
                                        break;
                                    }
                                    ...
                                    let temp_file_path = temp_dir
                                        .join(format!("{}_{}.mp4.part", dev_id, request_id));
                                    ...
                                    if let Err(e) = write_res {
                                        tracing::error!(
                                            "Failed to write video chunk payload: {:?}",
                                            e
                                        );
                                        continue;
                                    }
                                    ...
                                    if (frame.flags & 0x1) != 0 {
                                        ...
                                        if std::fs::rename(&temp_file_path, &final_file_path)
                                            .is_ok()
                                        {
                                            ...
                                            let size_bytes = final_file_path
                                                .metadata()
                                                .map(|m| m.len())
                                                .unwrap_or(0);
                                ...
                                2 => {
                                    ...
                                    let request_id =
                                        frame.meta.get("request_id").and_then(|r| r.as_str());

                                    if let Some(req_id) = request_id {
                                        if !is_valid_request_id(req_id) {
                                            tracing::error!(
                                                "Invalid request_id in photo frame: {:?}",
                                                req_id
                                            );
                                            break;
                                        }
